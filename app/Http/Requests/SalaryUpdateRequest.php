...
class SalaryUpdateRequest extends FormRequest
{
    ...
    {
        return $this->user()->can('update gaji');
    }
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            // Get the salary being updated from the URL parameter
            $salaryId = $this->route('gaji');

            if ($salaryId) {
                // If the route parameter is the model instance (route model binding)
                $currentSalaryId = is_object($salaryId) ? $salaryId->id : $salaryId;

                // Check for duplicate salary period (exclude current record)
                $existingSalary = Salary::where('employee_id', $this->employee_id)
                    ->where('id', '!=', $currentSalaryId)
